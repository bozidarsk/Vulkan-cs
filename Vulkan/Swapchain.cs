...
public sealed class Swapchain : IDisposable
{
	...
	public unsafe Image[] GetImages()
	{
		...
		return images.Select(x => new Image(x, device, allocator)).ToArray();
	...
	public string Name
	{
		set
		{
			...
			using var nameInfo = new DebugUtilsObjectNameInfo(
				...
				objectHandle: (ulong)(nint)swapchain,
	...
	public void Dispose()
	{
		vkDestroySwapchainKHR(device.Handle, swapchain, allocator?.Handle ?? default);
